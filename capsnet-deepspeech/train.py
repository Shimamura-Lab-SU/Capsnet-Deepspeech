...
sys.path.append("../deepspeech.pytorch-master")
...
parser.add_argument('--log-dir', default='visualize/deepspeech_final', help='Location of tensorboard log for wer,cer')
#parser.add_argument('--log-dirit', default='visualize/deepspeech_final', help='Location of tensorboard log for loss text')
...
if __name__ == '__main__':
    ...
    if args.continue_from:  # Starting from previous model
        ...
        model = DeepSpeech(rnn_hidden_size=args.hidden_size,
                           ...
                           labels=labels,batch_size=args.batch_size,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, (data) in enumerate(train_loader, start=start_iter): 
            ...
            if i == len(train_sampler):
                break
            #入力
            nowitr = epoch *len(train_sampler) + i   
            ...
            if nowitr % 200  == 0:
                ...
                for size in target_sizes:
                    ...
                    offset += size
                tar_str = decoder.convert_to_strings(split_targets)
            ...
            avg_loss += loss_value
            losses.update(loss_value, inputssize)
            #del loss
            ...
            if( nowitr % 200 == 0 ):
                ...
                outstr, _ =decoder.decode(out.to(device).transpose(0,1), output_sizes)
                
                
                j = 0
                #for j in range(len(tar_str)):
                writer.add_text('text', outstr[0][0] +'222' + tar_str[0][0] ,nowitr )
                print(outstr[0][0] +'222' + tar_str[0][0] )    
                
                twer , tcer , lenw ,lenc  = 0 , 0 , 0 , 0 
                for x in range(len(tar_str)):
                    #pdb.set_trace()
                    twer += decoder.wer(outstr[x][0],tar_str[x][0])
                    tcer += decoder.cer(outstr[x][0],tar_str[x][0])
                    lenw += len(tar_str[x][0].split())
                    lenc += len(tar_str[x][0].replace(' ', ''))
                
                writer.add_scalar('wer_train_epochs' ,float(twer)/lenw ,nowitr)
                writer.add_scalar('cer_train_epochs' ,float(tcer)/lenc ,nowitr)
               
                del outstr,tar_str
                
            del  loss,out, input_sizes , output_sizes
        ...
        writer.add_scalar('wer_epochs',wer,epoch +1)
        writer.add_scalar('cer_epochs',cer,epoch +1)
        values = {
            'loss_results': loss_results,
