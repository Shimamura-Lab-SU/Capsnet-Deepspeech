...
"""
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, (data) in enumerate(train_loader, start=start_iter): 
            ...
            del float_out,targets, target_sizes
            ...
            writer.add_scalar('loss_iter',losses.avg,epoch *len(train_sampler) + i+1)
            ...
            if(i  == 0 ):
                outstr, _ =decoder.decode(out.transpose(0,1), output_sizes)
                
                
                j = 0
                for j in range(len(tar_str)):
                    writer.add_text('text[{0}]'.format(j), outstr[j][0] +'222' + tar_str[j][0] ,epoch )
                ...
                writer.add_scalar('wer_train_epochs' ,float(twer)/lenw ,epoch+1)
                writer.add_scalar('cer_train_epochs' ,float(tcer)/lenc ,epoch+1)
