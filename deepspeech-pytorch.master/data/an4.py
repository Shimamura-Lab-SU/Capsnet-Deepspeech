...
parser.add_argument('--target-dir', default='E:\\deepspeech.pytorch-master/data/an4_dataset', help='Path to save dataset')
...
def _format_data(root_path, data_tag,  wav_folder):
...
def _convert_audio_to_wav(train_path):
    ...
    with os.popen('wsl find %s -type f -name "*.raw"' % train_path) as pipe:
        for line in pipe:
            raw_path = line.strip()
...
def main():
    ...
    _format_data(root_path, 'test' , 'an4test_clstk')
    _format_data(root_path,'train' , 'an4_clstk')
