...
parser.add_argument("--tar-path",default='TEDLIUM_release2.tar.gz', type=str, help="Path to the TEDLIUM_release tar if downloaded (Optional).")
...
def cut_utterance(src_sph_file, target_wav_file, start_time, end_time, sample_rate=16000):
   ...
   #                                                                   target_wav_file, start_time, end_time)],
    #                shell=True)
    #print(src_sph_file,str(sample_rate),target_wav_file,start_time)
    sph =SPHFile(src_sph_file)
    sph.write_wav(target_wav_file, start_time, end_time)
...
def main():
    ...
    print(train_ted_dir, target_unpacked_dir, args.tar_path)
